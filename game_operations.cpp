...
void stratego_operations::battle(boardChip player1_piece, boardChip player2_piece) {
	int winner = 0;
	if ((isdigit(player1_piece.piece) != 0) && (isdigit(player2_piece.piece) != 0)) {
		...
		else if (player1_piece.piece == 'f') {
			winner = 2;
		}
		else if (player2_piece.piece == 'f') {
			winner = 1;
		}
	}
	...
	if (winner == 1) {
		for (int i = 0; i < 40; i++) {
			if ((player2_pieces[i].x_position == player2_piece.x_position) && (player2_pieces[i].y_position == player2_piece.y_position)) {
		...
		for (int i = 0; i < 40; i++) {
			if ((player1_pieces[i].x_position == player1_piece.x_position) && (player1_pieces[i].y_position == player1_piece.y_position)) {
...
bool stratego_operations::wait_for_player1_game_loop() {
	...
	cout << "\nPress 1 to save as game1, 2 as game2, 3 as game3, and 4 as game4." << endl;
...
bool stratego_operations::select_piece_and_move() {
	...
	if ((selected_piece.piece != ' ') && (selected_piece.piece != 'b') && (selected_piece.piece != 'f') && ((board[curser_y_position][curser_x_position])[1] != 'X')) {
		while (input != ESC) {
			...
			else if ((input == ENTER) || (scout_moved == true)) {
				if (player_turn == 1) {
					if (direction == '^') {
						if (board_chip_clear(curser_x_position, curser_y_position - distance)) {
							...
							hide_player1();
							show_player2();
							battle(player1_piece, player2_piece);
							add_curser_to_board();
							move_player1_piece_at_curser_position_up(distance);
							hide_player2();
							system("CLS");
					...
					if (direction == '^') {
						...
						if (board_chip_clear(curser_x_position + distance, curser_y_position)) {
							...
							boardChip player2_piece = get_piece_at_curser_position();
							boardChip player1_piece = get_piece_at_position(curser_x_position + 1, curser_y_position);
							reveal_piece(player1_piece);
							system("CLS");
							display_board();
							cout << "Press enter to continue." << endl;
		...
		if (input == ESC) {
			for (int i = 0; i < 10; i++) {
				for (int j = 0; j < 10; j++) {
					if (((board[i][j])[1] == '^') || ((board[i][j])[1] == 'v') || ((board[i][j])[1] == '<') || ((board[i][j])[1] == '>')) {
						(board[i][j])[1] = ' ';
					}
				}
			}
...
bool stratego_operations::menu_loop() {
	...
	while (load_success == false) {
		cout << "Press 0 for a new game or 1, 2, 3, or 4 to load a saved game." << endl;
