...
void stratego_operations::move_curser_position(int direction) {
	if (((player_turn == 1) && (direction == UP)) || ((player_turn == 2) && (direction == DOWN))) {
	...
	else if (((player_turn == 1) && (direction == DOWN)) || ((player_turn == 2) && (direction == UP))) {
...
void stratego_operations::set_up_handler(int command) {
	...
	else if ((command == RIGHT) || (command == LEFT) || (command == UP) || (command == DOWN)) {
...
void stratego_operations::display_player1_out_of_play_pieces() {
	...
	string display = "";
	printw("Player 1's pieces out of play: ");
	for (int i = 0; i < 40; i++) {
...
void stratego_operations::display_player2_out_of_play_pieces() {
	int ones = 0, twos = 0, threes = 0, fours = 0, fives = 0, sixes = 0, sevens = 0, eights = 0, nines = 0, spys = 0, bombs = 0, flags = 0;
	string display = "";
	printw("Player 2's pieces out of play: ");
	for (int i = 0; i < 40; i++) {
...
int stratego_operations::determine_winner(boardChip player1_piece, boardChip player2_piece) {
	...
	if ((isdigit(player1_piece.piece) != 0) && (isdigit(player2_piece.piece) != 0)) {
		...
		else if ((player1_piece.piece == 's') && ((player2_piece.piece != player2_powerful_piece) || (player2_piece.piece == 'f'))) {
			winner = 2;
		}
		else if ((player2_piece.piece == 's') && (player1_piece.piece == player1_powerful_piece)) {
			winner = 2;
		}
		else if ((player2_piece.piece == 's') && ((player1_piece.piece != player1_powerful_piece) || (player1_piece.piece == 'f'))) {
			winner = 1;
		}
...
bool stratego_operations::wait_for_player1_game_loop() {
	...
	int input2 = 0;
	erase();
	printw("Player 1's turn. Press Space to continue\n");
	printw("\n1 -> save as game 1\n2 -> save as game 2\n3 -> save as game 3\n4 -> save as game 4\n");
...
bool stratego_operations::wait_for_player2_game_loop() {
	...
	int input2 = 0;
	erase();
	printw("Player 2's turn. Press Space to continue\n");
	printw("\n1 -> save as game 1\n2 -> save as game 2\n3 -> save as game 3\n4 -> save as game 4\n");
	while ((input != SPACE) && (input != ONE) && (input != TWO) && (input != THREE) && (input != FOUR)) {
		input = getch();
	}
...
void stratego_operations::game_loop() {
	...
	if (player_turn == 1) {
		...
		if (game_loaded) {
			erase();
			display_saved_board();
			printw("Opponent's move. Press enter to continue.\n");
			do {
				input5 = getch();
			} while (input5 != ENTER);
			game_loaded = false;
	...
	printw("Press Enter to view player 1's pieces.\n");
	while (input4 != ENTER) {
		input4 = getch();
	}
	player_turn = 1;
	...
	show_player1();
	erase();
	printw("Player 1's pieces.\n");
	display_board();
	printw("Press Enter to view player 2's pieces.\n");
	while (input2 != ENTER) {
		input2 = getch();
	}
	player_turn = 2;
	...
	show_player2();
	erase();
	printw("Player 2's pieces.\n");
	display_board();
	printw("Press Enter to end game.\n");
	while (input3 != ENTER) {
		input3 = getch();
	}
}
...
int stratego_operations::move_scout_up() {
	...
	while ((input != ENTER) && (input != Q)) {
		input = getch();
		if (((input == UP) && (player_turn == 1) && !player1_piece_at_location(curser_x_position, scout_y_position - 1) && !player2_piece_at_location(curser_x_position, scout_y_position)) || ((input == DOWN) && (player_turn == 2) && !player2_piece_at_location(curser_x_position, scout_y_position - 1) && !player1_piece_at_location(curser_x_position, scout_y_position))) {
		...
		else if (((input == DOWN) && (player_turn == 1) && !player2_piece_at_location(curser_x_position, scout_y_position + 1) && !player1_piece_at_location(curser_x_position, scout_y_position)) || ((input == UP) && (player_turn == 2) && !player2_piece_at_location(curser_x_position, scout_y_position + 1) && !player1_piece_at_location(curser_x_position, scout_y_position))) {
...
int stratego_operations::move_scout_down() {
	...
	while ((input != ENTER) && (input != Q)) {
		...
		else if (((input == DOWN) && (player_turn == 1) && !player1_piece_at_location(curser_x_position, scout_y_position + 1) && !player2_piece_at_location(curser_x_position, scout_y_position)) || ((input == UP) && (player_turn == 2) && !player2_piece_at_location(curser_x_position, scout_y_position + 1) && !player1_piece_at_location(curser_x_position, scout_y_position))) {
			if ((scout_y_position != 9) && ((scout_y_position != 3) || ((curser_x_position != 2) && (curser_x_position != 3) && (curser_x_position != 6) && (curser_x_position != 7)))) {
				if (!((player_turn == 2) && ((scout_y_position + 1) >= curser_y_position))) {
					if ((player_turn == 1) && ((board[scout_y_position][curser_x_position])[1] == ' ')) {
						(board[scout_y_position][curser_x_position])[1] = 'v';
...
int stratego_operations::move_scout_right() {
	...
	while ((input != ENTER) && (input != Q)) {
		input = getch();
		if (((input == RIGHT) && (player_turn == 1) && !player1_piece_at_location(scout_x_position + 1, curser_y_position) && !player2_piece_at_location(scout_x_position, curser_y_position)) || ((input == RIGHT) && (player_turn == 2) && !player2_piece_at_location(scout_x_position + 1, curser_y_position) && !player1_piece_at_location(scout_x_position, curser_y_position))) {
...
int stratego_operations::move_scout_left() {
	...
	while ((input != ENTER) && (input != Q)) {
		input = getch();
		if (((input == LEFT) && (player_turn == 1) && !player1_piece_at_location(scout_x_position - 1, curser_y_position) && !player2_piece_at_location(scout_x_position, curser_y_position)) || ((input == LEFT) && (player_turn == 2) && !player2_piece_at_location(scout_x_position - 1, curser_y_position) && !player1_piece_at_location(scout_x_position, curser_y_position))) {
	...
	while (load_success == false) {
		...
		if (input == ONE) {
			...
			if (load_success == false) {
				printw("There is no game saved for that game number.\n"); 
				input = 0;
			}
			else {
				return false;
...
bool stratego_operations::move_piece_up_routine(boardChip selected_piece) {
	...
	if (not_no_man_zone(UP) && clear_of_own_piece(UP)) {
...
bool stratego_operations::move_piece_down_routine(boardChip selected_piece) {
	...
	int input = 0;
	bool scout_enter = false;
	if (not_no_man_zone(DOWN) && clear_of_own_piece(DOWN)) {
...
bool stratego_operations::move_piece_right_routine(boardChip selected_piece) {
	...
	int input = 0;
	bool scout_enter = false;
	if (not_no_man_zone(RIGHT) && clear_of_own_piece(RIGHT)) {
...
bool stratego_operations::move_piece_left_routine(boardChip selected_piece) {
	...
	int input = 0;
	bool scout_enter = false;
	if (not_no_man_zone(LEFT) && clear_of_own_piece(LEFT)) {
...
void stratego_operations::display_move_result(int player) {
	...
	erase();
	display_board();
	printw("Press enter to end turn.\n");
	do {
		input = getch();
	} while (input != ENTER);
}
...
string stratego_operations::getExePath(){
